...
class PatchFeedDataManager{
    func patchFeed(_ feedText : String?, _ viewController : UIViewController, completion: @escaping () -> Void){
        ...
        AF.request("\(Constant.BASE_URL)/feeds/\(Constant.FEED_ID)", method: .patch, parameters: param ,encoding: JSONEncoding.default, headers: headers)
            .responseDecodable(of: PatchFeedModel.self){response in
                ...
                case .success(let response):
                    ...
                    case 2902:
                        viewController.dismissIndicator()
                        viewController.presentAlert(title: "피드 문구는 최소 1자부터 최대 1000자 이내로 입력해야합니다.")
                        print("피드 문구는 최소 1자부터 최대 1000자 이내로 입력해야합니다.")
                    case 2904:
                        viewController.dismissIndicator()
